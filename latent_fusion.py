"""Memory-conscious helpers for the frozen-price/simple-text-fusion experiment.
...
in streaming parquet batches, pools articles within each stock-date, and fits
the simple concatenation classifier from the paper draft.
"""
...
class SimpleConcatenationFusion(nn.Module):
    """Simple concatenation followed by a configurable residual MLP head.

    The modalities still meet only through ``[h_price; h_text]``. Residual
    blocks deepen the post-concatenation map without introducing gating or
    cross-attention, so this remains the simple-concatenation model family.
    """
    ...
    def __init__(
        ...
        self.price_norm = nn.LayerNorm(price_dim)
        self.fuse_input = nn.Sequential(
            nn.Linear(price_dim + text_dim, hidden_dim),
            nn.GELU(),
            nn.Dropout(dropout),
        )
        self.residual_blocks = nn.ModuleList([
            ResidualMLPBlock(hidden_dim, expansion, dropout) for _ in range(depth)
        ])
    ...
    def forward(
        ...
        fused = self.fuse_input(torch.cat([self.price_norm(price), h_text], dim=1))
...
def fit_fusion_model(
    ...
    weight_decay: float = 1e-4,
    seed: int = 42,
) -> tuple[SimpleConcatenationFusion, list[dict[str, float]]]:
    ...
    model = SimpleConcatenationFusion(
        price.shape[1], family_text.shape[2], family_text.shape[1], hidden_dim,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for start in range(0, len(target), batch_size):
            ...
            optimizer.zero_grad(set_to_none=True)
            loss = F.binary_cross_entropy_with_logits(model(p, t, m), y)
...
def _assemble_fusion_arrays(
    ...
    # SimpleConcatenationFusion.forward().
...
def run_walk_forward_fusion(
    *,
    ...
    """Train fold-local coders/fusion heads, report OOS metrics, and refit final models."""
    ...
    for family in requested_families:
        ...
        try:
            ...
            catalog_rows.append({"family": family, "usable": True, "input_dim": input_dim, "status": "ok"})
        except Exception as error:
            catalog_rows.append({"family": family, "usable": False, "input_dim": None, "status": str(error)})
    if not families:
        raise RuntimeError("No usable original embedding family was found")
    pl.DataFrame(catalog_rows, infer_schema_length=None).write_csv(output_dir / "text_family_catalog.csv")
    variants = {family: (family,) for family in families}
    variants["all_families"] = tuple(families)
    feature_set = "timesfm_original_text_fusion"
    ...
    for fold in fold_numbers:
        ...
        row_paths = _scope_text_latents(
            ...
            fit_targets, families, latent_dim, device, text_epochs, text_batch_size,
            force_refresh, run_training,
        )
        for variant, variant_families in variants.items():
            train_index, train_price, train_text, train_mask, train_y = _assemble_fusion_arrays(
                train_ids, train_price_latents, train_targets, row_paths, variant_families
            )
            val_index, val_price, val_text, val_mask, val_y = _assemble_fusion_arrays(
                validation_ids, train_price_latents, train_targets, row_paths, variant_families
            )
            model_name = f"timesfm_plus_{variant}"
            config_id = f"{model_name}__00"
            params = {
                ...
                "families": list(variant_families),
            }
            params_json = json.dumps(params, sort_keys=True)
            model_path = (
                fold_dir / "fusion_models" /
                f"{model_name}_resmlp_d{fusion_depth}.pt"
            )
            if model_path.exists() and not force_refresh:
                checkpoint = torch.load(model_path, map_location="cpu", weights_only=False)
                model = SimpleConcatenationFusion(
                    train_price.shape[1], latent_dim, len(variant_families),
                    fusion_hidden_dim, depth=fusion_depth,
                    expansion=residual_expansion, dropout=fusion_dropout,
                )
                model.load_state_dict(checkpoint["state_dict"]); model.eval()
            else:
                if not run_training:
                    raise FileNotFoundError(f"{model_path} is absent; enable training")
                model, history = fit_fusion_model(
                    train_price, train_text, train_mask, train_y, device,
                    hidden_dim=fusion_hidden_dim, depth=fusion_depth,
                    expansion=residual_expansion, dropout=fusion_dropout,
                    epochs=fusion_epochs,
                    batch_size=fusion_batch_size, seed=seed + int(fold),
                )
                save_torch_model(model_path, model, {
                    "scope": f"fold_{fold}", "model": model_name,
                    "price_encoder": "timesfm", "price_encoder_frozen": True,
                    "text_families": list(variant_families), "equation": "simple_concatination",
                    "fusion_depth": fusion_depth,
                    "residual_expansion": residual_expansion,
                    "fusion_dropout": fusion_dropout,
                })
                pl.DataFrame(history).write_csv(
                    fold_dir / "fusion_models" / f"{model_name}_training_history.csv"
                )
            score = predict_fusion(model, val_price, val_text, val_mask, device)
            metric_rows.append(_metric_row(
                feature_set, model_name, config_id, params_json, fold,
                len(train_y), val_y, score,
            ))
            prediction_parts.append(val_index.with_columns(
                ...
                pl.Series("y_pred", (score >= 0.5).astype(np.int8)),
                pl.lit(0.5).alias("decision_threshold"),
            ).drop("target_up"))
            del model, train_price, train_text, train_mask, val_price, val_text, val_mask
            gc.collect()
            if device == "mps":
                torch.mps.empty_cache()
        fold_table = pl.DataFrame([row for row in metric_rows if row["fold"] == fold])
        fold_table.write_csv(fold_dir / f"selected_model_metrics_{feature_set}.csv")

    fold_metrics = pl.DataFrame(metric_rows).sort(["model", "fold"])
    oof_predictions = pl.concat(prediction_parts).select([
        "feature_set", "model", "config_id", "fold", "row_id", "date", "ticker",
        "y_true", "y_score", "y_pred", "decision_threshold",
    ...
    oof_predictions.write_parquet(
        output_dir / f"selected_config_oof_predictions_{feature_set}.parquet", compression="zstd"
    )

    # Refit every text coder and fusion variant on all labeled training rows.
    ...
    scoped_test_links = test_links.with_columns(
        (pl.col("row_id").cast(pl.UInt64) + test_row_offset).alias("row_id")
    )
    ...
    final_row_paths = _scope_text_latents(
        "final", final_dir, data_dir, train_links, scoped_test_links, final_targets,
        families, latent_dim, device, text_epochs, text_batch_size,
        force_refresh, run_training,
    )
    all_train_ids = final_targets.select("row_id")
    all_test_ids = scoped_test_price.select("row_id")
    final_prediction_parts, final_metric_rows = [], []
    for variant, variant_families in variants.items():
        train_index, train_price, train_text, train_mask, train_y = _assemble_fusion_arrays(
            all_train_ids, train_price_latents, train_targets, final_row_paths, variant_families
        )
        # Test labels are retained only for evaluation; they never enter fitting.
        test_index, test_price, test_text, test_mask, test_y = _assemble_fusion_arrays(
            all_test_ids, scoped_test_price, scoped_test_targets, final_row_paths,
            variant_families, require_target=False,
        )
        model_name = f"timesfm_plus_{variant}"
        model_path = (
        ...
        if model_path.exists() and not force_refresh:
            ...
            save_torch_model(model_path, model, {
                "scope": "final", "model": model_name, "price_encoder": "timesfm",
                "price_encoder_frozen": True, "text_families": list(variant_families),
                "equation": "simple_concatination",
                "fusion_depth": fusion_depth,
                "residual_expansion": residual_expansion,
                "fusion_dropout": fusion_dropout,
            })
        ...
        score = predict_fusion(model, test_price, test_text, test_mask, device)
        final_prediction_parts.append(test_index.with_columns(
            (pl.col("row_id") - test_row_offset).cast(pl.UInt32).alias("row_id"),
            pl.lit(model_name).alias("model"),
            pl.Series("y_score", score.astype(np.float32)),
            pl.Series("y_pred", (score >= 0.5).astype(np.int8)),
        ))
        labeled_test = np.isfinite(test_y)
        if labeled_test.any():
            final_metric_rows.append(_metric_row(
                feature_set, model_name, f"{model_name}__00", "{}", 0,
                len(train_y), test_y[labeled_test], score[labeled_test],
            ))
        del model, train_price, train_text, train_mask, test_price, test_text, test_mask
        gc.collect()
        if device == "mps":
            torch.mps.empty_cache()
    final_predictions = pl.concat(final_prediction_parts).sort(["model", "date", "ticker"])
    final_metrics = pl.DataFrame(final_metric_rows, infer_schema_length=None)
    final_predictions.write_parquet(output_dir / "final_test_predictions.parquet", compression="zstd")
    final_metrics.write_csv(output_dir / "final_test_metrics.csv")

    # Build direct fold-level and aggregate comparisons with saved baselines.
    baseline_tables = []
    for path in sorted(baseline_dir.glob("*/selected_config_fold_metrics_*.csv")):
        baseline_tables.append(pl.read_csv(path))
    comparison_folds = pl.concat([*baseline_tables, fold_metrics], how="diagonal_relaxed")
    comparison_aggregate = _aggregate_fold_metrics(comparison_folds.select(fold_metrics.columns))
