...
$\mathcal Q_{\mathrm{ROC}}$ & Exact candidate thresholds induced by the pooled inner-validation prediction scores when selecting $q^\star$. \\
...
are also fitted within the fold. Each outer training scope contains three
expanding, purged inner-validation windows. Hyperparameters are selected by
their mean inner-window balanced accuracy, checkpoints minimize
class-balanced validation BCE, and the refit epoch count is the median of the
three window-specific best epochs. One decision threshold is selected from
the pooled inner-window predictions before the chosen architecture is refitted
on all observations available in the outer training scope. The same
inner-selection procedure is repeated on the complete labeled training set
...
The active fusion models use class-balanced binary cross-entropy. For
$\mathcal B_+$ and $\mathcal B_-$ denoting the positive and negative examples
in a training scope, respectively,
...
-
\frac{1}{2|\mathcal B_+|}
\sum_{(i,t)\in\mathcal B_+}\log p_{i,t}
-
\frac{1}{2|\mathcal B_-|}
\sum_{(i,t)\in\mathcal B_-}\log(1-p_{i,t}).
\end{equation}

Checkpoint selection minimizes class-balanced validation BCE, while Optuna
selects hyperparameters by mean inner-window balanced accuracy under the
pooled inner-validation threshold.
...
Model selection is nested inside every outer training fold. Specifically,
three nonoverlapping chronological inner-validation windows are formed. Their
training sets expand through time, and each training--validation boundary has
a 20-trading-date purge. Numeric scalers are fitted separately using only the
training observations of the corresponding inner window. Every candidate is
trained with class-balanced BCE and checkpointed using class-balanced
validation BCE. Its selection score is the mean balanced accuracy across the
three windows under one threshold fitted from their pooled predictions.
...
where $y_{\mathrm{inner}}$ and $\widehat p_{\mathrm{inner}}$ concatenate the
labels and predictions from all three purged inner-validation windows.
...
\begin{itemize}
    ...
    \item early stopping based on class-balanced validation BCE;
