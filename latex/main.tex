...
\begin{quote}
Can pretrained time-series models be adapted through inspectable historical
regimes while retaining competitive decision performance and providing exact,
sparse, and stable case-based explanations?
\end{quote}
...
where $\ell$ may be a point or probabilistic forecasting loss. The competition
decision is instead
...
Within the training portion of each chronological fold, construct
...
where each prototype is an actual stock--date observation containing a
historical price window, its permitted contextual embeddings, and its
directional label. Cache
...
These global weights expose the contribution of price and each embedding
family to case retrieval, without assigning semantic meaning to individual
...
The main intrinsically interpretable variant constrains
...
A positive prototype can therefore provide only positive evidence and a
negative prototype only negative evidence. An ablation removes this constraint
to measure its predictive cost.
...
There is no parallel unrestricted MLP head. Define the exact contribution
...
The explanation identifies which historical regimes the model used and their
exact numerical contributions. It is a faithful account of the model
calculation, not a claim that those regimes caused the future return.
...
\begin{enumerate}
    ...
    \item train prototype adapters, modality weights, distance scales, and
