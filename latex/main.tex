...
fine-tuning. Price representations are combined with the supplied text
embedding vectors using gated fusion or cross-attention. A cross-sectional
Transformer can additionally model dependencies among stocks at each
prediction date.
...
$x^C_{i,t}\in\R^{F_C}$ & Complete engineered covariate vector available at forecast origin $t$: market and volume features, calendar and Fourier features, and text-availability counts or indicators. In the implemented TimesFM fusion model, $F_C=106$. \\
...
$h^{T,k}_{i,t,j}$; $h^T_{i,t}$; $H^T_{i,t}$ & Projected representation of item $j$ from family $k$, a single pooled text vector, and a sequence of $J$ text tokens, respectively. \\
$J$; $d$ & Number of text tokens and shared latent width used by the multimodal model. \\
\bottomrule
\end{longtable}
...
$h^C_{i,t}$ & Joint market representation obtained by passing $[h^P_{i,t};\bar x^C_{i,t}]$ through the residual covariate--price MLP. \\
...
$W_T,b_T,\phi_T$ & Text-projection matrix, bias, and nonlinear projection mapping. \\
$Q^T,K^P,V^P$ & Text queries and price keys/values used in text-to-price cross-attention. \\
...
The supplied embedding vectors are projected into a shared latent dimension:
...
If several embedding families or several temporal text aggregates are used,
the text representation is retained as multiple tokens:
...
\subsection{Residual Covariate--Price Encoder with Text Fusion}

The implemented frozen-TimesFM model does not send the pooled pretrained
vector directly to the text-fusion head.  It first combines that vector with
all engineered covariates available at the forecast origin.  Let

\begin{equation}
h^P_{i,t}
=
\operatorname{Pool}\!\left(H^P_{i,t}\right)
\end{equation}

be the frozen TimesFM representation, and let $\bar x^C_{i,t}$ be the
engineered covariate vector after training-fold median imputation and
standardization.  The initial joint market state is
...
W_C\operatorname{LayerNorm}\!\left(
[h^P_{i,t};\bar x^C_{i,t}]
\right)+b_C
\right)
\right).
\end{equation}

For $\ell=1,\ldots,B_C$, a pre-normalized residual MLP block updates this
state as
...
\operatorname{GELU}\!\left(
W^{C,(\ell)}_1
\operatorname{LayerNorm}\!\left(h^{C,(\ell-1)}_{i,t}\right)
+b^{C,(\ell)}_1
\right)
\right)
+b^{C,(\ell)}_2,\\
...
&=
h^{C,(\ell-1)}_{i,t}
+\operatorname{Dropout}\!\left(v^{C,(\ell)}_{i,t}\right).
\end{align}

The resulting market representation is
$h^C_{i,t}=\operatorname{LayerNorm}(h^{C,(B_C)}_{i,t})$.  Thus, the residual
market encoder learns interactions between the frozen TimesFM latent vector
and the tabular market, volume, calendar, Fourier, and text-availability
covariates.

For multiple text-embedding families, let $a^T_{i,t}\in\{0,1\}^{J}$ denote
family availability and set unavailable family vectors to zero.  The text
branch used by the simple-concatenation implementation is

\begin{equation}
h^{T,\mathrm{pool}}_{i,t}
=
...
The two branches are concatenated only after the residual market encoding:
...
\operatorname{Dropout}\!\left(
\operatorname{GELU}\!\left(
W_F[h^C_{i,t};h^{T,\mathrm{pool}}_{i,t}]+b_F
\right)
\right).
\label{simple_covariate_concatenation}
\end{equation}

A second sequence of $B_F$ pre-normalized residual MLP blocks, defined in the
same form as above with fusion-specific parameters, produces $u^{(B_F)}_{i,t}$.
The directional probability is

\begin{equation}
p_{i,t}
=
\sigmoid\!\left(
w_o^\top\operatorname{LayerNorm}\!\left(u^{(B_F)}_{i,t}\right)+b_o
\right).
\end{equation}

TimesFM and the cached $h^P_{i,t}$ remain frozen.  Text coders, covariate
preprocessing, both residual MLP stacks, and the classifier are fitted within
each expanding training fold.  Hyperparameters and the decision threshold are
selected from out-of-sample fold predictions only; the selected model is then
refitted using all labeled training observations.
...
The two principal price-encoder choices are summarized by

\begin{equation}
h^P_{i,t}
=
\begin{cases}
\operatorname{TCN}\left(X^P_{i,t-L+1:t}\right),
& \text{trained from scratch},\\[2mm]
...
For the implemented frozen-TimesFM concatenation model, the pooled price
vector is first augmented with the engineered covariates and encoded jointly:

\begin{equation}
h^{P+C}_{i,t}
=
\operatorname{ResidualMLP}_C\!\left(
[h^P_{i,t};\bar x^C_{i,t}]
\right).
\end{equation}

The concatenation variant therefore uses $h^{P+C}_{i,t}$ rather than the
unaugmented $h^P_{i,t}$.  More generally, after the price history has been
encoded, either price encoder can be paired with each of the same three
text-fusion choices:
...
=
\begin{cases}
[h^{P+C}_{i,t};h^T_{i,t}],
& \text{concatenation},\\
[\widetilde h^P_{i,t};\widetilde h^T_{i,t}],
& \text{elementwise gating},\\
\operatorname{CrossAttention}\left(H^T_{i,t},H^P_{i,t}\right),
& \text{cross-attention}.
\end{cases}
\end{equation}

Consequently, the TCN and frozen-TimesFM paths each produce three distinct
model variants rather than one generic ``model with text.''  When permitted,
the same three fusion variants can also be evaluated with a LoRA-adapted
TimesFM encoder in place of the frozen encoder.

A Temporal Fusion Transformer (TFT) may additionally be trained from scratch
using only competition-provided price variables and text embeddings.  This
does not introduce another pretrained model: all TFT variable-selection,
gating, recurrent, attention, and classification parameters are initialized
and learned on the competition training data.  Any known-future inputs used by
the TFT must likewise be supplied or deterministically available at forecast
time; unavailable future market or text information cannot be used.
...
allows an MLP to learn interactions, but it compresses the temporal sequence
before the text is introduced.

Cross-attention preserves the temporal tokens and allows the text
representation to select relevant historical price patterns. Text-to-price
cross-attention is defined by
...
Here $W_Q$, $W_K$, and $W_V$ are learned projection matrices and
$d_{\mathrm{att}}$ is the per-head query/key dimension used for attention
scaling. The superscripts $T$ and $P$ label the text and price modalities;
only $\top$ denotes transpose.

This operation asks:

\begin{quote}
Given the current text representation, which portions of the historical price
trajectory are relevant for the directional prediction?
\end{quote}

A bidirectional variant additionally lets price tokens attend to text tokens:
...
\begin{itemize}
    ...
    \item frozen TimesFM price encoder with concatenated text;
    \item frozen TimesFM price encoder with elementwise gated text fusion;
    \item frozen TimesFM price encoder with text-to-price cross-attention;
...
\begin{enumerate}
    \item frozen TimesFM representation concatenated with engineered
    covariates, encoded by a residual MLP, and fused with projected text;
    \item frozen representation plus gated text fusion;
    \item frozen representation plus text-to-price cross-attention;
...
The initial high-priority model is
...
H^T_{i,t}
&=
\operatorname{TextProjector}
\left(
z^T_{i,t}
\right),\\
u_{i,t}
&=
\operatorname{GatedFusion}
\left(
\operatorname{Pool}(H^P_{i,t}),
\operatorname{Pool}(H^T_{i,t})
\right),\\
p_{i,t}
&=
\sigmoid
...
The first implementation will keep TimesFM frozen. The subsequent versions
will add:

\begin{enumerate}
    \item text-to-price cross-attention;
