...
$P_k=(X_k^P,c_k,y_k)$; $p_k$ & The $k$th prototype, which is an actual training observation, and its frozen pooled TSFM representation. \\
$K$; $K_0$ & Number of prototypes and maximum number of prototypes activated for one observation, with $K_0\ll K$. \\
$d_{i,t,k}$; $r_{i,t,k}$ & Multimodal distance from observation $(i,t)$ to prototype $k$, and the corresponding sparse routing weight. \\
$A_k,B_k$ & Rank-$r$ adapter matrices associated with prototype $k$. \\
$z_{i,t,k}$; $\beta_k$; $C_{i,t,k}$ & Post-adaptation prototype evidence, prototype coefficient, and exact contribution $\beta_kz_{i,t,k}$ to the directional logit. \\
...
$\mathcal L_{\mathrm{forecast}}$ & Mean forecasting loss over the current mini-batch; $\ell$ denotes the selected point or probabilistic forecasting loss. \\
$\mathcal L_{\mathrm{margin}}$ & Logistic margin loss for directional prediction. \\
...
$\mathcal Q_{\mathrm{ROC}}$ & Exact candidate thresholds induced by the inner-validation prediction scores when selecting $q^\star$. \\
...
jointly within each expanding outer training fold. Imputation and scaling
parameters are also fitted within the fold. Each outer training scope is split
once more into an inner-training period, a purge interval, and one
inner-validation period. Hyperparameters are selected by minimizing ordinary
inner-validation BCE. Early stopping inside an Optuna trial is used only to
evaluate that hyperparameter configuration; its epoch count and checkpoint
weights are not carried into the final fit. After Optuna selects all non-epoch
hyperparameters, a fresh calibration run selects the epoch minimizing
ordinary inner-validation BCE and the decision threshold maximizing
inner-validation balanced accuracy. The calibration checkpoint is then
discarded and the chosen architecture is initialized again and trained on all
observations available in the outer training scope for exactly that epoch
count. The same procedure is repeated on the complete labeled training set
before the final model is refitted on all labeled observations.
...
The active fusion models optimize ordinary mean binary cross-entropy:
...
Checkpoint selection and Optuna hyperparameter selection minimize ordinary
validation BCE. The decision threshold is selected only after hyperparameter
selection by maximizing balanced accuracy on the inner-validation predictions.
...
Model selection is nested inside every outer training fold. Specifically, the
outer-training portion is split chronologically into one inner-training
period, a 20-trading-date purge, and one inner-validation period. Numeric
scalers are fitted using only the inner-training observations. Every candidate
is trained with ordinary BCE and checkpointed using ordinary
inner-validation BCE. Its Optuna objective is the negative ordinary BCE on
that inner-validation period (equivalently, the study minimizes that BCE).
Once Optuna has selected the non-epoch hyperparameters, a
separate fresh calibration on the same inner split selects the epoch count and
final threshold. That checkpoint is discarded; only the selected epoch count
and threshold are retained for the refit on the complete outer-training
portion.
...
q^\star
=
\arg\max_{q\in\mathcal Q_{\mathrm{ROC}}}
\operatorname{BalancedAccuracy}
\left(
y_{\mathrm{inner}},
\ind[\widehat p_{\mathrm{inner}}\geq q]
\right),
\end{equation}

where $y_{\mathrm{inner}}$ and $\widehat p_{\mathrm{inner}}$ are the labels and
predictions from the purged inner-validation period.
$\mathcal Q_{\mathrm{ROC}}$ contains the exact thresholds induced by those
scores; it is not restricted to a fixed probability interval. Ties are
resolved in favor of the threshold closest to $0.5$.
...
\begin{itemize}
    ...
    \item early stopping based on ordinary validation BCE;
...
LoRA-adapted multimodal foundation model, a prototype-routed interpretable
model, or an ensemble of these approaches. The prototype-routed variant adds a
separate methodological objective: each directional logit must be reconstructed
exactly from a sparse set of inspectable historical cases, without an
unrestricted prediction path that can bypass the explanation.
...
The methodological objective is to make adaptation of a pretrained time-series
foundation model both decision-specific and intrinsically case-interpretable.
The central research question is:

\begin{quote}
Can a pretrained time-series model achieve competitive directional performance
when both its adaptation and its final prediction are constrained to pass
through a sparse set of real, inspectable historical cases?
\end{quote}

Directly optimizing a binary directional loss is standard classification, and
input-dependent low-rank adaptation is already studied in parameter-efficient
fine-tuning \citep{toplora}. Prototypes have also been used to explain
time-series predictions \citep{obermair2023prototype}. The proposed
contribution is therefore their integration under a strict architectural
constraint: inspectable historical prototypes both route the low-rank
adaptation and form the only evidence available to the final decision head.
There is no unrestricted prediction path that can bypass the prototype
bottleneck.

\subsection{Forecasting Objective and Directional Decision}

Let $Y_{i,t}$ be the future return for observation $(i,t)$ and let
$\widehat Y_{i,t}$ be its numerical forecast. A conventional forecasting model
minimizes
...
\frac{1}{B}
\sum_{(i,t)\in\mathcal B}
\ell\left(\widehat Y_{i,t},Y_{i,t}\right),
\end{equation}

where $\mathcal B$ is a minibatch of $B=|\mathcal B|$ stock--date observations
and $\ell$ may be a point or probabilistic forecasting loss. The competition
decision is instead
...
A small numerical forecasting error can reverse the sign when $Y_{i,t}$ is
close to zero, whereas a large magnitude error may leave the sign unchanged.
The primary model is therefore trained directly for direction. The forecasting
loss remains an auxiliary ablation rather than a claimed methodological
contribution.

\subsection{Frozen Representation and Inspectable Prototype Bank}

A pretrained TSFM with frozen parameters $\theta_0$ produces
...
h^0_{i,t}
=
\operatorname{Pool}(H^0_{i,t})
\in\R^d.
\end{equation}

Here, $X^P_{i,t}$ is the historical price window for stock $i$ at forecast
origin $t$; $f_{\theta_0}$ is the pretrained TSFM with frozen parameters
$\theta_0$; $H^0_{i,t}$ contains its $M$ token representations of width $d$;
and $h^0_{i,t}$ is their pooled representation. Within the training portion of
each chronological fold, construct
...
where $K$ is the number of prototypes, $X_k^P$ is the historical price window
of the $k$th prototype, $c_k=\{c_k^{(m)}\}_{m=1}^{K_T}$ is its collection of
permitted contextual embeddings across the $K_T$ supplied embedding families,
and $y_k\in\{-1,+1\}$ is its realized directional label. Thus, each $P_k$ is an
actual stock--date observation rather than a free latent vector. Cache its
frozen pooled TSFM representation as
...
The bank is initialized using class-balanced $K$-medoids in the frozen
representation space. Unlike a free latent prototype, every $P_k$ can be
inspected as an observed historical price window. Prototype selection is
repeated inside every outer fold and never uses validation or future data.

\subsection{Multimodal Distance and Sparse Routing}

Define the price-representation distance
...
For supplied text-embedding family $m$, define
...
\|c^{(m)}_{i,t}\|_2\|c^{(m)}_k\|_2+\epsilon
}.
\end{equation}

Here, $c^{(m)}_{i,t}$ and $c^{(m)}_k$ are respectively the query and prototype
embeddings from supplied family $m$, and $\epsilon>0$ prevents division by
zero. The combined distance is
...
\omega_m\geq0,
\qquad
\omega_P+\sum_{m=1}^{K_T}\omega_m=1.
\end{equation}

The modality weights are learned rather than manually specified. Introduce
global trainable logits $\alpha_P,\alpha_1,\ldots,\alpha_{K_T}$ and set

\begin{equation}
\left(
\omega_P,\omega_1,\ldots,\omega_{K_T}
\right)
=
\operatorname{softmax}
\left(
\alpha_P,\alpha_1,\ldots,\alpha_{K_T}
\right).
\end{equation}

Thus, $\omega_P$ is the learned global price-distance weight and $\omega_m$ is
the learned global weight for contextual embedding family $m$. These weights
expose the contribution of price and each embedding family to case retrieval,
without assigning semantic meaning to individual embedding coordinates.

Only the $K_0$ closest prototypes may affect one observation:
...
\left(
-d_{i,t,1},\ldots,-d_{i,t,K}
\right).
\end{equation}

The corresponding routing weights are
...
Here, $\tau>0$ is the routing temperature, and $r_{i,t,k}$ is the normalized
weight assigned to prototype $k$ for observation $(i,t)$. Thus,

\begin{equation}
\|\mathbf r_{i,t}\|_0\leq K_0,
\qquad
\sum_{k=1}^{K}r_{i,t,k}=1.
\end{equation}

Routing weights are inspectable but are not claimed to be faithful
explanations by themselves; attention weights alone need not explain a
prediction \citep{jain2019attention}.

\subsection{Prototype-Routed Low-Rank Adaptation}

Prototype $k$ owns a rank-$r$ representation adapter
...
The routed adapted representation is
...
H^0_{i,t}
+
\sum_{k\in\mathcal S_{i,t}}
r_{i,t,k}
H^0_{i,t}A_k^\top B_k^\top,
\qquad
\widetilde h_{i,t}
=
\operatorname{Pool}(\widetilde H_{i,t}).
\end{equation}

The rule-conservative implementation applies this adapter outside the frozen
backbone. If backbone adaptation is explicitly permitted, the same
prototype router can activate adapters at selected internal layers.

\subsection{No-Bypass Additive Decision Bottleneck}

After adaptation, the evidence supplied by prototype $k$ is
...
Here, $\sigma_k>0$ is the median frozen-representation distance from prototype
$k$ to the training-fold observations assigned to its $K$-medoids cluster,
with a small positive floor for degenerate clusters. Thus, it introduces no
additional trainable parameter.
The quantity $z_{i,t,k}\in(0,1]$ measures post-adaptation similarity for an
active prototype and is zero for an inactive one. The main intrinsically
interpretable variant constrains

\begin{equation}
\beta_k
=
y_k\operatorname{softplus}(\eta_k).
\end{equation}

Here, $\eta_k$ is an unconstrained learned scalar and $\beta_k$ is the resulting
sign-constrained coefficient. A positive prototype can therefore provide only
positive evidence and a negative prototype only negative evidence. An ablation
removes this constraint to measure its predictive cost.

The final directional logit and probability are

\begin{align}
s_{i,t}
&=
b+\sum_{k=1}^{K}\beta_kz_{i,t,k},
\\
p_{i,t}
&=
\sigmoid(s_{i,t}).
...
Here, $b$ is a scalar intercept, $s_{i,t}$ is the directional logit, and
$p_{i,t}$ is the predicted probability of the positive class. There is no
parallel unrestricted MLP head. The core trainable parameter collection is
...
\cup
\left\{
b,\alpha_P,\alpha_1,\ldots,\alpha_{K_T}
\right\}.
\end{equation}

The pretrained parameters $\theta_0$, prototype observations $P_k$, cached
prototype representations $p_k$, and training-fold-derived bandwidths
$\sigma_k$ remain fixed. The routing temperature $\tau$ is selected using
inner chronological validation. Define the exact contribution
...
The explanation identifies which historical prototype cases the model used
and their exact numerical contributions. It is a faithful account of the model
calculation, not a claim that those cases caused the future return. The method
is intrinsically case-interpretable and contribution-faithful, but it does not
assign semantic meaning to individual embedding dimensions or automatically
explain why two contextual embeddings are similar.

\subsection{Directional Training Objective}

The model minimizes the unweighted logistic margin loss
...
Every stock--date observation receives equal weight, matching the directional
hit-rate criterion. Capacity is controlled by $(K,K_0,r)$, adapter norm
control, and chronological early stopping.

\subsection{Formal Interpretability Properties}

\paragraph{Exact completeness.}
The explanation vector
$C_{i,t}=(C_{i,t,1},\ldots,C_{i,t,K})$ satisfies
...
It reconstructs the complete pre-sigmoid prediction, with no residual from a
hidden decision head.

\paragraph{Exact sparsity.}
Because $z_{i,t,k}=0$ outside $\mathcal S_{i,t}$,
...
\paragraph{Intervention consistency.}
Under the explicit intervention that sets $z_{i,t,k}$ to zero while holding
the remaining evidence fixed, the resulting logit $s_{i,t}^{(-k)}$ obeys
...
Let $\xi_{i,t}$ be the concatenated normalized price and context
representation used by the distance function, and suppose every
$d_k(\xi)$ is $L_d$-Lipschitz. Define the retrieval gap
...
where the subscripts denote ordered distances. If $\Delta(\xi)>0$ and
...
then $\mathcal S(\xi')=\mathcal S(\xi)$. Each selected distance can increase
by at most $L_d\|\xi'-\xi\|_2$, while each unselected distance can decrease by
at most the same amount, so the ordering across the selection boundary cannot
reverse. This supplies a testable sufficient condition for explanation
stability.

\subsection{Leakage-Safe Training and Evaluation}

For each outer walk-forward fold:

\begin{enumerate}
    \item fit preprocessing and extract frozen representations using only the
    outer training period;

    \item select class-balanced prototype medoids from that period;

    \item train prototype adapters, modality-weight logits, and additive
    coefficients;

    \item select $K$, $K_0$, $r$, and $\tau$ using inner chronological
    validation;

    \item freeze the pipeline and evaluate it once on the outer future period.
\end{enumerate}

Predictive evaluation reports hit rate, binary cross-entropy, parameter count,
and fold-to-fold variability. Interpretability evaluation reports
completeness, active-prototype count, comprehensiveness after removing the
largest contributions, sufficiency when retaining only the largest
contributions, Jaccard stability under small permissible perturbations,
prototype-neighborhood purity and diversity, temporal coverage, and
intervention consistency.

Required comparisons include a frozen linear probe, forecast-then-threshold,
an unrestricted MLP head, standard LoRA, token-wise or mixture-of-LoRA
adaptation, a prototype classifier without adaptation, and the proposed model
with and without the no-bypass bottleneck. The unrestricted-head ablation
quantifies the accuracy cost of intrinsic interpretability.

\subsection{Research Hypotheses and Claimed Contributions}

\begin{description}
    \item[H1.]
    Prototype-routed adaptation improves directional accuracy over a frozen
    prototype classifier and static LoRA under the same trainable-parameter
    budget.

    \item[H2.]
    The sparse additive bottleneck retains most of the accuracy of an
    unrestricted head while providing exact and more stable explanations.

    \item[H3.]
    Multimodal prototype retrieval improves prediction and regime
    identification relative to price-only or text-only retrieval.

    \item[H4.]
    Prototype routing improves performance across chronological folds by
    localizing adaptation to historically similar regimes.
\end{description}

The proposed research makes three focused contributions:

\begin{enumerate}
    \item a sparse, case-routed low-rank adapter for pretrained time-series
    representations, grounded in actual historical observations;

    \item a no-bypass additive prototype bottleneck that provides exact,
    sparse, and intervention-consistent explanations of every directional
    logit;

    \item a leakage-safe evaluation framework that jointly measures predictive
    performance, explanation faithfulness, prototype quality, and stability
    under temporal shift.
\end{enumerate}
...
Let $Y_{i,t}$ denote the future quantity of interest for observation $(i,t)$
and let $\widehat Y_{i,t}$ denote its model forecast. Over a mini-batch
$\mathcal B$, a conventional forecasting model minimizes the mean numerical
loss
...
For each $(i,t)\in\mathcal B$, let
$y_{i,t}=2\ind\{Y_{i,t}>0\}-1\in\{-1,+1\}$ be the realized directional label
and let $s_\theta(X_{i,t})\in\R$ be the model's signed directional score. The
decision-aligned surrogate is
...
The loss therefore encourages directional agreement and weights every
observation equally, matching the unweighted directional hit-rate criterion.
...
A relational alternative preserves pairwise token geometry. Let
$Z\in\R^{M\times d}$ have nonzero token rows $z_1,\ldots,z_M$. Define its
pairwise cosine-similarity matrix $S(Z)\in\R^{M\times M}$ entrywise as
...
The relational consistency loss is then
...
so the diagonal entries of each similarity matrix are one, whereas its
off-diagonal entries encode similarities between distinct tokens.
...
\begin{enumerate}
    ...
    \item A margin-based surrogate objective that directly targets directional
    correctness.
...
\bibitem[Li et al.(2025)]{toplora}
Li, S., Luo, X., Wang, H., Tang, X., Cui, Z., Liu, D., Li, Y., He, X.,
and Li, R.
\newblock Beyond higher rank: Token-wise input--output projections for
efficient low-rank adaptation.
\newblock \emph{Advances in Neural Information Processing Systems}, 2025.

\bibitem[Obermair et al.(2023)]{obermair2023prototype}
Obermair, C., Fuchs, A., Pernkopf, F., Felsberger, L., Apollonio, A.,
and Wollmann, D.
\newblock Example or prototype? Learning concept-based explanations in
time-series.
\newblock \emph{Proceedings of the 14th Asian Conference on Machine Learning},
2023.
